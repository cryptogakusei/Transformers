...
class PageAllocator:
    ...
    def allocate(self, new_keys, new_values, layer, request_id):
        ...
        while (space_needed > 0):
            ...
            if current_page_space_left > 0:
                ...
                for loc in range(start_pos_page, end_pos_page):
                    mem_assignment_per_round.append((current_page, loc))
        ...
        return mem_assignment_per_round

            
    # get the KV cache during the forward pass of the model
    def get_cache(self, layer, request_id):
        
        if (request_id, layer) not in self.page_table:
            k_cache = torch.empty(1, self.num_heads, 0, self.head_dim, device=self.page_pool_keys.device)
            v_cache = torch.empty(1, self.num_heads, 0, self.head_dim, device=self.page_pool_values.device)
    ...
    def check_space_availability(self, request_ids, num_new_tokens_per_id, num_layers, num_speculations):
        ...
        for i, request_id in enumerate(request_ids):
            ...
            total_extra_pages_needed += num_speculations # added speculations
    ...
    def reclaim_cellwise(self, mem_loc, request_id, layer):
        # mem_loc is [(page, cell), ....] format
        # mark the page_table, page_space_left and empty_pages appropriately

        # get list of all pages touched
        pages_touched = []
        for (page,_) in mem_loc:
            pages_touched.append(page)
        pages_touched = list(set(pages_touched))

        if len(pages_touched) > 1:
            # clean up all pages except the first one
            for page in pages_touched[1:]:
                self.page_space_left.pop(page, None)
                self.empty_pages.append(page)
                self.page_table[(request_id, layer)].remove(page)                

        # exception case for the first one
        count = sum(1 for (page, _) in mem_loc if page == pages_touched[0])
        self.page_space_left[pages_touched[0]] = count
