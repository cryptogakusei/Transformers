...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x, partition, active_requests, page_allocator, mem_assignment_draft_model):
        ...
        for seq_range, request_id in partition.items():
            ...
            mem_assignment_per_round = page_allocator.allocate(
                                        new_keys=keys[:,:,start_pos:end_pos,:], 
                                        new_values=values[:,:,start_pos:end_pos,:], 
                                        layer=self.layer,
                                        request_id=request_id)
            
            # accounting for speculative decoding
            if (request_id, self.layer) not in mem_assignment_draft_model:
                mem_assignment_draft_model[(request_id, self.layer)] = []
            mem_assignment_draft_model[(request_id, self.layer)].append(mem_assignment_per_round)
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x, partition, active_requests, page_allocator, mem_assignment_draft_model):
        ...
        x = self.attention(x, partition, active_requests, page_allocator, mem_assignment_draft_model) # -- same here --
...
class DraftModel(nn.Module):
    ...
    def forward(self, in_idx, partition, active_requests, page_allocator, mem_assignment_draft_model):
        x = self.tok_emb(in_idx) # batch_size x 1 x emb_dim
        x = self.dropout_emb(x) # -- same here --
        for block in self.blocks:
            x = block(x, partition, active_requests, page_allocator, mem_assignment_draft_model) # -- same here --
