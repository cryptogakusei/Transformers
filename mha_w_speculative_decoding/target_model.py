...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x, partition, active_requests, page_allocator, temp_new_kvcache_target_model):
        ...
        for seq_range, request_index in partition.items():
            ...
            # store in the temporary KV_cache
            new_keys = keys[:,:,start_pos:end_pos,:]
            new_values=values[:,:,start_pos:end_pos,:]

            if (request_id, self.layer) not in temp_new_kvcache_target_model:
                temp_new_kvcache_target_model[(request_id, self.layer)] = []
            temp_new_kvcache_target_model[(request_id, self.layer)].append((new_keys, new_values))

            # create the full KV cache to be used for computing attention matrix
            slice_keys, slice_values = page_allocator.get_cache(layer=self.layer, request_id=request_id) # retrieves what is there in page allocator
            slice_keys = torch.cat((slice_keys, new_keys), dim=2)
            slice_values = torch.cat((slice_values, new_values), dim=2) 
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x, partition, active_requests, page_allocator, temp_new_kvcache_target_model):
        ...
        x = self.attention(x, partition, active_requests, page_allocator, temp_new_kvcache_target_model) # -- same here --
...
class TargetModel(nn.Module):
    ...
    def forward(self, in_idx, partition, active_requests, page_allocator, temp_new_kvcache_target_model):
        x = self.tok_emb(in_idx) # batch_size x 1 x emb_dim
        x = self.dropout_emb(x) # -- same here --
        for block in self.blocks:
            x = block(x, partition, active_requests, page_allocator, temp_new_kvcache_target_model) # -- same here --
