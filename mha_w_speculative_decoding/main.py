...
# instantiate page allocators for both models
...
for step in range(5):
    ...
    print(f'Is space available in the page allocator of draft?: {speculative_decoding_manager.draft_page_allocator.check_space_availability(request_ids, num_new_tokens_per_id, MODEL_CONFIG["num_layers"], INFERENCE_CONFIG["num_speculations"])}')
    print(f'Is space available in the page allocator of target?: {speculative_decoding_manager.target_page_allocator.check_space_availability(request_ids, num_new_tokens_per_id, MODEL_CONFIG["num_layers"], INFERENCE_CONFIG["num_speculations"])}')
...
while any(request.status != "completed" for request in schedular.active_requests.values()):
    inference_seq, partition, active_requests = schedular.next_seq()
...
for i, request in enumerate(schedular.active_requests.values()):
