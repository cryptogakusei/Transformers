...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x, partition, mask, active_requests, page_allocator):
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x, partition, mask, active_requests, page_allocator):
        ...
        x = self.attention(x, partition, mask, active_requests, page_allocator) # -- same here --
...
class MHAModelPagedAttention(nn.Module):
    ...
    def forward(self, in_idx, partition, mask, active_requests, page_allocator):
        x = self.tok_emb(in_idx) # batch_size x 1 x emb_dim
        x = self.dropout_emb(x) # -- same here --
        for block in self.blocks:
            x = block(x, partition, mask, active_requests, page_allocator) # -- same here --
