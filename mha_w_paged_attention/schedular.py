...
class Schedular:
    ...
    def queue(self, context_length, num_layers, prompt_token_id, max_new_tokens, request_id=None):
        request = ActiveRequest(context_length, num_layers, prompt_token_id, max_new_tokens, request_id)
        self.active_requests.append(request)

    def next_seq(self, page_allocator):
        inference_seq = None
        ...
        for i, request in enumerate(self.active_requests):
            if request.status == "decoding":
                ...
                for col in range(num_col, num_col + page_allocator.cache_len(request.request_id) + 1):
                ...
                num_tok += 1
                num_col += page_allocator.cache_len(request.request_id) + 1 # applicable for KVCache class, 1 is there because we will add the new token too
        ...
        for i, request in enumerate(self.active_requests):
            if (request.status == "prefill") and (num_tok < self.max_num_batched_tokens):    
                ...
                for row in range(num_tok,  num_tok + tokens_to_add):
                    for col in range(num_col , num_col + page_allocator.cache_len(request.request_id) + (row - num_tok) + 1):
                ...
                num_col += page_allocator.cache_len(request.request_id) + tokens_to_add
        ...
        print(f"Batch: {num_tok} tokens | Decode: {sum(1 for request in self.active_requests if request.status == 'decoding')} | Prefill: {sum(1 for request in self.active_requests if request.status == 'prefill')} | Partition: {partition}")
