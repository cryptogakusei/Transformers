...
### Applying RoPE
# the implementation follows the eqn 34 in https://arxiv.org/pdf/2104.09864 
class RoPE(nn.Module):
    def __init__(self, head_dim, rope_limit, base=10000):
        super().__init__()
        self.rope_limit = rope_limit
        ...
        position = torch.arange(rope_limit).unsqueeze(1) # rope_limit x 1
        angles =  position * theta_interleave # rope_limit x head_dim

        self.register_buffer('cos', torch.cos(angles)) # rope_limit x head_dim
        self.register_buffer('sin', torch.sin(angles)) # rope_limit x head_dim       
